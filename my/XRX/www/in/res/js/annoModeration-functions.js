...
function geti18nText(lang, key){
    ...
    xmlhttp.open("GET","anno/service/get-i18n-text?key="+key+"&amp;lang="+lang,false);
...
else if(idPrefix == "rep")
  {
  var rid = "repimg"+id;
  var queryid = "#repimg"+id;
  var view = "viewsrep"+id;
  } 
...
function resetAnnoFields(){
    ...
    var allpub = document.getElementById("allReports").childNodes;
    var number = 0;
    ...
    for(i=1;i<=number+1;i++)
        ...
        if(document.getElementById(parentID) != null)
        	...
        	var index = 0;
		      ...
		      for(p=1;p<=index;p++)
        				{
        				onmouseover = onmouseover+"jQuery('#repannofield"+i+"-"+p+"').css('border-color','blue');";
              	onmouseout = onmouseout+"jQuery('#repannofield"+i+"-"+p+"').css('border-color','#EF6A2F');";
        				}
		      for(u=1;u<=index;u++)
        			{
				      var field = "repannofield"+i+"-"+u;
				      if(document.getElementById(field) != null)
        				{
				        document.getElementById(field).setAttribute('onmouseover', onmouseover);
				        document.getElementById(field).setAttribute('onmouseout', onmouseout);
...
function loadInfoTool(id, user, tagid, type, registerid, folio, lang){
  ...
  if (window.XMLHttpRequest)
	...
	if(type == 'request')     
  	xmlhttp.open("GET","anno/service/get-annotation-data?id="+id+"&user="+user+"&type=request&registerid="+registerid+"&folio="+folio,true);
  else
  ...
  if (xmlhttp.readyState==4)
     ...
     if (xmlhttp.status==200)
        ...
        jQuery('#deleteSurface').css('display', 'block');
        ...
        if(type == 'request')
        	...
        	var index = 0;
		      ...
		      for(i=1;i<=index;i++)
        		 {
        		 var fieldID = 'repannofield'+tagid+'-'+i;
        		 if(document.getElementById(fieldID) != null)
        				{
		        		 jQuery('#'+fieldID).css('border-color','blue');
		        		 document.getElementById(fieldID).setAttribute('onmouseover', '');
        	...
        	var editstring = "prepareEditAnno('"+id+"', '"+user+"', 'report', '"+lang+"', '"+registerid+"', '"+folio+"')";
	        var editSurfacestring = "prepareEditSurface('"+id+"', '"+user+"', '"+tagid+"', 'report', '"+registerid+"', '"+folio+"', '"+lang+"')";
	        var addstring = "prepareAddSurface('"+id+"', '"+user+"', '"+tagid+"', 'report', '"+registerid+"', '"+folio+"', '"+lang+"')";
	        document.getElementById("editButton").setAttribute('ONCLICK', editstring);
	        document.getElementById("editSurfaceButton").setAttribute('ONCLICK', editSurfacestring);
	        document.getElementById("addButton").setAttribute('ONCLICK', addstring);
       ...
       error++;
       if(error < 2)
          loadInfoTool(id, user, tagid, status, type, registerid, folio, lang);
       else
          {
          error = 0;
          }   
...
var back = "loadInfoTool('"+id+"', '"+user+"', '"+tagid+"', '"+type+"', '"+registerid+"', '"+folio+"', '"+lang+"')";
       ...
       if(document.getElementById(field) != null)
        	{
        	 onmouseover = "";
		       onmouseout = "";
		       onclick = "loadInfoTool('"+id+"', '"+user+"', '"+tagid+"', '"+type+"', '"+registerid+"', '"+folio+"', '"+lang+"')";
					 document.getElementById(field).setAttribute('onmouseover', onmouseover);
					 document.getElementById(field).setAttribute('onmouseout', onmouseout);
   ...
   var index = 0;
	 ...
	 for(x=0;x<allFields.length;x++)
       ...
       if(type == "request")
       	...
       	var field = "repannofield"+tagid+"-"+p;
       if(document.getElementById(field) != null)
         {
...
if(type == "request")
	{
	var imgID = 'pubimg'+tagid;
	}
else
	{
	var imgID = 'repimg'+tagid;
	}
...
function saveMove(id, user, tagid, field, type, registerid, folio, lang){
    ...
    xmlhttp.open("POST","anno/service/edit-request-surface?id="+id+"&user="+user+"&type="+type,true);
    xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
    xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState==4)
       {
       if (xmlhttp.status==200)
          ...
          if (response == "edit-surface")
              ...
              loadInfoTool(id, user, tagid, type, registerid, folio, lang);
...
if(document.getElementById(parentID) != null)
   ...
   var index = 0;
	 ...
	 for(x=0;x<allFields.length;x++)
       ...
       if(document.getElementById(field) != null)
         {
	       var  onmouseover = "", onmouseout = "", onclick = "loadInfoTool('"+id+"', '"+user+"', '"+tagid+"', '"+type+"', '"+registerid+"', '"+folio+"', '"+lang+"')";
				 document.getElementById(field).setAttribute('onclick', onclick);
				 document.getElementById(field).setAttribute('onmouseover', onmouseover);
				 document.getElementById(field).setAttribute('onmouseout', onmouseout);
				 jQuery('#'+field).css('display', 'block');
...
if(index == 1) 
  ...
  if(confirm(geti18nText(lang, 'delete-surface-question')))
    { 
		...
		xmlhttp.open("POST","anno/service/delete-request-surface?id="+id+"&user="+user+"&type="+type,true);
		xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
		xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4)
		   {
...
function addSurface(id, user, tagid, type, registerid, folio, lang){
    ...
    xmlhttp.open("POST","anno/service/add-request-surface?id="+id+"&user="+user+"&type="+type,true);
    xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
    xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState==4)
       {
       if (xmlhttp.status==200)
          ...
          if (response == "add-surface")
              ...
              // functions
              var onclick = "loadInfoTool('"+id+"', '"+user+"', '"+tagid+"', '"+type+"', '"+registerid+"', '"+folio+"', '"+lang+"')";
                    
              // define style and mouse actions
              if(type == "request")
			       		var fieldid = "pubannofield"+tagid+"-"+fieldIdNumber;
			       	else
			       		var fieldid = "repannofield"+tagid+"-"+fieldIdNumber;
              var fieldstyle = "left:"+x1+"px;top:"+y1+"px;height:"+x2+"px;width:"+y2+"px;border-color:blue;";
              ...
              // load data into editor
              if(type == 'request')
                loadInfoTool(id, user, tagid, 'request', registerid, folio, lang);
              else
                loadInfoTool(id, user, tagid, 'report', registerid, folio, lang);
              // hide taskBar and show editTaskBar
							jQuery('#taskBar').css('display', 'block');
							jQuery('#editTaskBar').css('display', 'none');
...
function updateAnno(id, user, type, lang, registerid, folio){
        ...
        if(type == 'request') 
                xmlhttp.open("POST","anno/service/edit-request-annotation?user="+user+"&id="+id+"&type=publish",true);
        else
                xmlhttp.open("POST","anno/service/edit-request-annotation?user="+user+"&id="+id+"&type=report",true);
        xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
        xmlhttp.onreadystatechange=function(){
        if (xmlhttp.readyState==4)
            {
...
function answerRequest(type, dataid, requestid, user, lang, registerid, folio){
    ...
    xmlhttp.open("POST","anno/service/answer-request?type="+type+"&id="+dataid+"&user="+user+"&folio="+folio+"&registerid="+registerid, true);
    xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
    xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState==4)
          {
           if (xmlhttp.status==200)
                 { 
                      ...
                      var repfieldid = "pubfield"+requestid
                      document.getElementById(deleteid).style.display = "none";
                      document.getElementById(repfieldid).style.display = "none";
                 ...
                 error++;
                 if(error < 20)
                   answerRequest(type, dataid, requestid, user, lang, registerid, folio);
                 else
                   {
                   jQuery('#loadgif').css('display', 'none');
                   document.getElementById("loadtext").innerHTML = geti18nText(lang, 'ajax-error');
...
// answer a report
function answerReport(dataid, requestid, user, lang, registerid, folio){
    var comment = jQuery('#repInput'+requestid).val();
    var deleteAnno;
    if(jQuery('#deleteAnno'+requestid).attr('checked') == 'checked')
                       deleteAnno = true;
                     else
                       deleteAnno = false;
    // send request to compare script
    ...
    xmlhttp.open("POST","anno/service/answer-report?&id="+dataid+"&user="+user+"&registerid="+registerid+"&folio="+folio+"&delete="+deleteAnno, true);
    xmlhttp.setRequestHeader("Content-type", "text/xml; charset=UTF-8");
    xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState==4)
          {
           if (xmlhttp.status==200)
                 { 
                      ...
                      var repfieldid = "repfield"+requestid
                      document.getElementById(deleteid).style.display = "none";
                      document.getElementById(repfieldid).style.display = "none";
                 ...
                 error++;
                 if(error < 20)
                   answerReport(dataid, requestid, user, lang, registerid, folio);
                 else
                   {
                   document.getElementById("loadtext").innerHTML = geti18nText(lang, 'ajax-error');
                   error = 0;
