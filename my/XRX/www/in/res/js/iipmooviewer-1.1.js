...
var IIP = new Class({
  ...
  scrollNavigation: function( e ) {
    ...
    if( e.event ){
      ...
      // From a drag
      xmove = e.offsetLeft * 2;
      ymove = (e.offsetTop-10) * 2;
  ...
  createWindows: function(){
    ...
    new Element('z', {src: '/in/iip.32x32.png', id: 'info', styles: { opacity: 0.8 } } ).injectInside('logo');

    // Fix IE7 PNG transparency problem
    if( Browser.Engine.trident5 ){
      $('info').setStyle( 'filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="/in/iip.32x32.png",sizingMethod=scale)' );
    }
  ...
  createNavigationWindow: function() {
	...
	src: this.server + '?FIF=' + this.images[0].src + '&SDS=' + this.images[0].sds + '&CNT=1.0' +
	    '&WID=' + this.min_x/2 + '&QLT=99&CVT=jpeg'
    });
   ...
   var loadBarContainer = new Element('div', {
	...
	id: 'navbuttons',
	html: '<img id="shiftLeft" src="/in/left.png"/><img id="shiftUp" src="/in/up.png"/><img id="shiftRight" src="/in/right.png"/><br/><img id="shiftDown" src="/in/down.png"/><br/><img id="zoomIn" src="/in/zoomIn.png"/><img id="zoomOut" src="/in/zoomOut.png"/><img id="reset" src="/in/reset.png"/>'
    });
  ...
  refreshLoadBar: function() {
    ...
    var w = (this.nTilesLoaded / this.nTilesToLoad) * this.min_x/2;
  ...
  positionZone: function(){
    ...
    var pleft = (this.rgn_x/this.wid) * (this.min_x/2);
    if( pleft > this.min_x/2 ) pleft = this.min_x/2;
    if( pleft < 0 ) pleft = 0;

    var ptop = (this.rgn_y/this.hei) * (this.min_y/2);
    if( ptop > this.min_y/2 ) ptop = this.min_y/2;
    if( ptop < 0 ) ptop = 0;

    var width = (this.rgn_w/this.wid) * (this.min_x/2);
    if( pleft+width > this.min_x/2 ) width = this.min_x/2 - pleft;

    var height = (this.rgn_h/this.hei) * (this.min_y/2);
    if( height+ptop > this.min_y/2 ) height = this.min_y/2 - ptop;

    if( width < this.min_x/2 ) this.xfit = 0;
    else this.xfit = 1;
    if( height < this.min_y/2 ) this.yfit = 0;
    else this.yfit = 1;
