...
declare variable $conf:platform-id := 
    substring-before(substring-after(request:get-uri(), '/'), '/');
declare variable $conf:db-base-collection-path := 
    concat('/db/www/', $conf:platform-id);
declare variable $conf:db-base-collection :=
    collection($conf:db-base-collection-path);
