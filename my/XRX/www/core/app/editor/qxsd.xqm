...
(:declare variable $qxsd:name := "editVDU";
declare variable $qxsd:default-relevant := fn:true();:)

declare variable $qxsd:default-search-id-in := '/db';
declare variable $qxsd:search-local-name := fn:true();
declare variable $qxsd:unbounded := 999;
declare variable $qxsd:default-max-depth := 10;
...
declare function qxsd:find-xsd($something)
...
{    
    let $node := 
        if($something instance of xs:string) then
            ...
            else if (collection($qxsd:default-search-id-in)/*[@id=$something]) then
                collection($qxsd:default-search-id-in)/*[@id=$something]    
            else ()
...
declare function qxsd:find-attribute($context, $name, $xsd){
    ...
    if($context/xs:attribute[xs:string(@name) = $name]) then
