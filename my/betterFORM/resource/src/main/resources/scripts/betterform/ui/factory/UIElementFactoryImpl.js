    ...
    createTextareaWidget:function(sourceNode, controlId, mediatype, appearance, classValue) {
    	...
    	if(mediatype && mediatype.toLowerCase() == "text/html") {
            ...
            return new betterform.ui.textarea.HtmlEditor({
                ...
                xfControlId:controlId,
                height:'100%',
                plugins:['undo', 'redo', '|', 'cut', 'copy', 'paste', '|', 'bold','italic','underline', 'strikethrough', '|', 'indent', 'outdent', 'justifyLeft', 'justifyRight', 'justifyCenter', '|', 'foreColor', '|','createLink', 'unlink', 'insertImage', '|', 'removeFormat', 'formatBlock', 'PasteFromWord'], 
                extraPlugins:[]
                }, sourceNode);
        }

	 else if(mediatype && mediatype.toLowerCase() == "text/xml") {
        	
        	if(!dojo.hasClass(sourceNode,"xfTextareaHTMLValue")){
                dojo.addClass(sourceNode,"")
            }
            
            /*dojo.require("vdu.XMLEditor");            
        	return new vdu.XMLEditor({
                name:controlId + "-value",
                services:dojo.attr(sourceNode,"services"),
                nodename:dojo.attr(sourceNode,"nodename"),
                xsdloc:dojo.attr(sourceNode,"xsdloc"),
                "class":classValue,
                title:dojo.attr(sourceNode,"title"),
                xfControlId:controlId
                }, sourceNode);*/
            
        	//console.log('///////////UIFACTORY.sourceNode', sourceNode.innerHTML);
        	
        	dojo.require("xrxa.AnnotationControl");            
        	return new xrxa.AnnotationControl({
                name:controlId + "-value",
                services:dojo.attr(sourceNode,"services"),
                nodename:dojo.attr(sourceNode,"nodename"),
                nodepath:dojo.attr(sourceNode,"nodepath"),
                namespace:dojo.attr(sourceNode,"namespace"),
                xsdloc:dojo.attr(sourceNode,"xsdloc"),
                rows:dojo.attr(sourceNode,"rows"),
                "class":classValue,
                title:dojo.attr(sourceNode,"title"),
                xfControlId:controlId
                }, sourceNode);
