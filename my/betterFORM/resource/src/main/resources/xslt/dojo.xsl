    ...
    <xsl:template name="buildControl">
       ...
       <!-- TODO: DateTime -->
        ...
        <xsl:choose>
		    ...
		    <!-- xrxa -->
                    <xsl:if test="@mediatype = 'text/xml'">
                        <xsl:attribute name="services"><xsl:value-of select="@services"/></xsl:attribute>
                        <xsl:attribute name="xsdloc"><xsl:value-of select="@xsdloc"/></xsl:attribute>
                        <xsl:attribute name="nodename"><xsl:value-of select="@nodename"/></xsl:attribute>
                        <xsl:attribute name="nodepath"><xsl:value-of select="@nodepath"/></xsl:attribute>
			...
			<!-- xrxa neccessary?-->
                        <xsl:when test="contains(@mediatype,'text/xml')">
                            <xsl:attribute name="tabindex" select="-1"/>
                        </xsl:when>
			<!-- xrxa -->
